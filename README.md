...
- 🏛️ **[معماری فنی سیستم (ARCHITECTURE.md)](ARCHITECTURE.md)**: دیاگرام‌های جریان سرتاسری، دریافت دوگانه، فالبک اضطراری پیامکی، صندوق OTP ردیس و ماشین وضعیت RPA.
- 🔌 **[مستندات کامل API سرور (API_DOCUMENTATION.md)](API_DOCUMENTATION.md)**: مشخصات وب‌هوک RPA، وب‌هوک درگاه مودم GSM، هدرها، کدهای پاسخ و کلیدهای ردیس.
- 📱 **[راهنمای جامع کاربری و استقرار (USER_GUIDE.md)](USER_GUIDE.md)**: راهنمای نصب رانندگان، تنظیم فالبک پیامکی، تنظیم وب‌هوک و دستورالعمل راه‌اندازی سرور.
...
- 🔒 **[سیاست‌های حریم خصوصی و امنیت (PRIVACY_POLICY.md)](PRIVACY_POLICY.md)**: اصول عدم ذخیره پیام‌های شخصی، ماسک‌کردن شماره، امنیت فالبک پیامکی و تفکیک داده‌ها.
...
### ۵. فالبک اضطراری پیامک به مودم سرور در قطعی اینترنت (Sub-Second SMS Fallback Relay)
- **پایش بی‌درنگ اتصال شبکه (`ConnectivityManager`)**: در صورت قطع کامل اینترنت جاده‌ای یا عدم اعتبارسنجی شبکه، کلاینت بلافاصله و با **۰ میلی‌ثانیه اتلاف وقت سوکت**، فالبک پیامکی را فعال می‌کند.
- **تایم‌اوت سخت‌گیرانه مسیر سریع (Fast-Path 1.5s Timeout)**: حتی در صورت وجود اینترنت ضعیف، تایم‌اوت درخواست شبکه روی ۱.۵ ثانیه محدود شده و در صورت عدم دریافت پاسخ یا فریز سوکت، کد فورا به عنوان پیامک اضطراری به مودم سرور ارسال می‌شود.
- **پروتکل ساختاریافته بارپرو**: ارسال فالبک در قالب استاندارد `BARPRO#driverId#driverPhone#smsType#code` با سرعت تحویل ۳ تا ۵ ثانیه به سیم‌کارت سرور.
- **پشتیبانی هوشمند از دو سیم‌کارت (Multi-SIM Routing)**: انتخاب خودکار اسلات سیم‌کارت مربوطه (`SubscriptionManager`) جهت ارسال فالبک با همان خطی که پیامک را دریافت کرده است.
- **پوشش کدهای سوخت و سهمیه پیمایش**: استخراج دقیق کدهای سامانه سهمیه سوخت، کارت هوشمند سوخت و نفت‌گاز علاوه بر سامانه‌های بارنامه کشوری (UTCMS).

### ۶. معماری آفلاین و ارسال تضمینی (Offline-First Resilient Pipeline)
...
### ۷. امنیت و رمزنگاری بانکی (Enterprise Security)
...
### ۸. رابط کاربری مدرن متریال دیزاین ۳ (Jetpack Compose UI)
- طراحی منطبق بر استانداردهای M3 با رنگ‌بندی اقیانوسی و طلایی لوکس.
- **صفحه مدیریت اختصاصی مجوزها (`PermissionManagerScreen`)**: تفکیک دسترسی‌های SMS (شامل ارسال و دریافت)، اعلان‌ها و پس‌زمینه با فیلترچیپ‌ها و آیکون‌های وضعیت.
...
- **تنظیمات یکپارچه سرور (`ServerConfigScreen`)**: تنظیم شماره مودم GSM فالبک سرور، اعتبارسنجی ارقام و تنظیم خودکار وب‌هوک RPA بارپرو با یک لمس.
...
## 📡 وب‌هوک دریافت پیامک مودم GSM و درگاه‌های پیامکی (Inbound SMS Relay)

این اندپوینت برای اتصال مودم‌های GSM لینوکسی متصل به سرور و همچنین وب‌هوک ورودی درگاه‌های پیامکی (کاوه‌نگار، مگفا، فراپیامک) طراحی شده و پیامک‌های فالبک اضطراری رانندگان را دریافت و مستقیماً وارد صندوق OTP ردیس می‌کند:
...
{
  "from": "09333702137",
  "text": "BARPRO#DRV-102938#09333702137#UTCMS_OTP#39182"
}
```
...
| `SEND_SMS` | ارسال پیامک فالبک اضطراری به مودم سرور در زمان قطعی اینترنت جاده‌ای | **SMS (الزامی)** |
